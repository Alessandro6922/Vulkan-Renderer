...
TODO:
	...
	probably move glm into the external folder
	and glfw maybe idk but probably
*/
...
private:
	...
	void createTextureImage() {
		...
		stbi_uc* pixels = stbi_load("Resources/Textures/texture.jpeg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
		...
		vkCmdCopyBufferToImage(commandBuffer, buffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 1, &region);
	...
	void drawFrame() {
		...
		if (result == VK_ERROR_OUT_OF_DATE_KHR) {
			...
			throw std::runtime_error("Dailed to acquire swap chain image!");
		}
